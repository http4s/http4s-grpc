...
object Messages {
  ...
  private def decodeLPMThroughDecoder[F[_]: MonadThrow, A](d: scodec.Decoder[A])(s: Stream[F, LengthPrefixedMessage]): Stream[F, A] = {
    s.evalMap(lpm =>
      liftAttempt(d.decodeValue(lpm.message.bits))
    )
  }

  private def decodeLPMStream[F[_]: RaiseThrowable](s: Stream[F, Byte]): Stream[F, LengthPrefixedMessage] = {
  ...
  private def encodeLPMThroughEncoder[F[_]: MonadThrow, A](e: scodec.Encoder[A])(s: Stream[F, A]): Stream[F, LengthPrefixedMessage] = {
    s
      .evalMap(a => liftAttempt(e.encode(a)))
      .map(b => LengthPrefixedMessage(false, b.bytes))
  }

  private def encodeLPMStream[F[_]: RaiseThrowable](s: Stream[F, LengthPrefixedMessage]): Stream[F, Byte] = {
